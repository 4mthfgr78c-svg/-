...
@dp.callback_query(F.data == 'admin_calc_couriers')
async def admin_calc_couriers(callback: types.CallbackQuery):
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute('SELECT tg_id, username, balance FROM users WHERE role = "courier" AND balance > 0')
    couriers = cursor.fetchall()
    conn.close()
    
    if not couriers:
        await callback.message.answer('💰 Нет выплат для курьеров')
        await callback.answer()
        return
    
    message = '💰 <b>Расчёт курьеров:</b>\n\n'
    for courier in couriers:
        message += f"👤 {courier[1] or courier[0]}: {courier[2]}₽\n"
    message += '\n<i>Скопируйте суммы и переведите курьерам</i>'
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='✅ Обнулить балансы', callback_data='reset_courier_balances')]
    ])
    
    await callback.message.answer(message, parse_mode='HTML', reply_markup=keyboard)
    await callback.answer()

@dp.callback_query(F.data == 'reset_courier_balances')
async def reset_courier_balances(callback: types.CallbackQuery):
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute('UPDATE users SET balance = 0 WHERE role = "courier"')
    conn.commit()
    conn.close()
    await callback.message.answer('✅ Балансы курьеров обнулены')
    await callback.answer()
